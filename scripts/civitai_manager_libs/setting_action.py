        ...
        with gr.Row():
            with gr.Accordion("Option", open=False):    
                with gr.Row():
                    ...
                    shortcut_max_download_image_per_version = gr.Slider(minimum=0, maximum=30, value=setting.shortcut_max_download_image_per_version, step=1,info="When a model shortcut is registered, you can specify the maximum number of images to download. \n This is the maximum number of images per version, a value of 0 disables the maximum to allow unlimited images.", label='Maximum number of images to download per version', interactive=True)
                with gr.Row():
